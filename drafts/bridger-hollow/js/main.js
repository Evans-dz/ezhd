/* ============================================================
   BRIDGER HOLLOW CREAMERY — interaction layer
...
(() => {
  ...
  const heroIntro = () => {
    ...
    tl.fromTo('.hL[data-depth="4"]', { yPercent: 8, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 1.6, ease: 'power3.out' }, 0)
      .fromTo('.hL[data-depth="3"]', { yPercent: 5, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 1.4, ease: 'power3.out' }, 0.08)
      .fromTo('.hL[data-depth="2"]', { yPercent: 3, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 1.2, ease: 'power3.out' }, 0.16)
  ...
  if (reduced) {
    ...
    const count = { v: 0 };
    const tl = gsap.timeline({ defaults: { ease: 'expo.out' } });
    tl.to('.mark-rim', { strokeDashoffset: 0, duration: 1.5, ease: 'power2.inOut' }, 0)
      .to('.mark-wedge', { strokeDashoffset: 0, duration: 1.0, ease: 'power2.inOut' }, 0.55)
      .to('.mark-eyes circle', { opacity: 1, scale: 1, duration: 0.45, stagger: 0.09, ease: 'back.out(2.5)' }, 0.95)
      .fromTo('.loader-word span', { yPercent: 120 }, { yPercent: 0, duration: 1.1, stagger: 0.04, ease: 'expo.out' }, 0.35)
  ...
  const burger = $('#burger');
  let menuOpen = false;
  const MENU_PARTS = ['.menu-bg', '.menu-links a > *', '.menu-media', '.menu-foot'];
  /* everything the open overlay covers — inert keeps Tab and screen readers inside the menu */
  const pageRegions = $$('#main, .footer, .mcta');
  ...
  const setMenu = (open) => {
    ...
    if (open) {
      if (lenis) lenis.stop();
      /* lenis is absent under reduced motion, so lock the document too */
      document.documentElement.style.overflow = 'hidden';
      menu.classList.add('is-open');
      if (reduced) {
        gsap.set('.menu-bg', { yPercent: 0 });
        gsap.set(['.menu-links a > *', '.menu-media', '.menu-foot'], { opacity: 1, y: 0 });
      } else {
      ...
      if (reduced) {
        ...
        gsap.timeline({ defaults: { ease: 'power2.in' } })
          ...
          /* a reopen within this timeline's ~0.76s would otherwise be un-opened by it */
          .add(() => { if (!menuOpen) menu.classList.remove('is-open'); });
      }
      if (lenis) lenis.start();
    }
  };
  ...
  $('#scrollCue')?.addEventListener('click', () => scrollTo('#story'));
  /* placeholder social links — don't let them jump the page to the top */
  ...
  if (!reduced) {
    ...
    /* clearProps drops the whole transform, so tilt cards keep theirs or they snap flat mid-hover */
    const playReveal = (el, i) => gsap.to(el, {
      opacity: 1, y: 0, duration: 1.1, delay: i * 0.08, ease: 'power3.out', overwrite: true,
      ...(el.hasAttribute('data-tilt') ? {} : { clearProps: 'transform' }),
    });
    ...
    /* Story — word-by-word scrub */
    const mText = $('#storyText');
    if (mText) {
    ...
    /* Parallax banners — scale gives headroom so edges never show */
    $$('[data-parallax]').forEach((wrap) => {
      const media = $('img, svg', wrap);
      const strength = parseFloat(wrap.dataset.parallax) || 12;
      gsap.fromTo(media, { yPercent: -strength / 2 }, {
        yPercent: strength / 2, ease: 'none',
        scrollTrigger: { trigger: wrap, start: 'top bottom', end: 'bottom top', scrub: true },
      });
    });

    /* Hero layered scene parallax */
    const depths = { 0: 0, 1: 4, 2: 7, 3: 12, 4: 18 };
    $$('.hL').forEach((layer) => {
      const d = depths[layer.dataset.depth] || 0;
      if (!d) return;
      gsap.to(layer, {
    ...
    /* Ambient life: clouds drift, sun breathes */
    gsap.to('#cloudA', { x: 90, duration: 46, repeat: -1, yoyo: true, ease: 'sine.inOut' });
    gsap.to('#cloudB', { x: -70, duration: 38, repeat: -1, yoyo: true, ease: 'sine.inOut' });
  }
  ...
  $$('[data-count]').forEach((el) => {
    /* data-since keeps "years running" stats honest without manual updates */
    if (el.dataset.since) el.dataset.count = String(new Date().getFullYear() - parseInt(el.dataset.since, 10));
  ...
  /* ---------------- Signature cheeses scrollytelling ---------------- */
  const makeImgs = $$('.make-img');
  const frameNum = $('#makesFrameNum');
  const setMake = (i, num) => {
    makeImgs.forEach((im, k) => im.classList.toggle('is-active', k === i));
    if (frameNum.textContent !== num) {
      frameNum.textContent = num;
      if (!reduced) gsap.fromTo(frameNum, { yPercent: 24, opacity: 0 }, { yPercent: 0, opacity: 0.85, duration: 0.7, ease: 'expo.out' });
    }
  };
  $$('.make').forEach((m, i) => {
    const num = $('.make-no', m).textContent;
    ScrollTrigger.create({
      trigger: m, start: 'top 62%', end: 'bottom 30%',
      onToggle: (self) => { if (self.isActive) setMake(i, num); },
    });
  });
  ...
  /* ---------------- Hero mouse drift (per layer) ---------------- */
  if (finePointer && !reduced) {
    const layers = $$('.hL').map((el) => ({
      el,
      k: [0, 3, 6, 10, 16][+el.dataset.depth] || 0,
      to: gsap.quickTo(el, 'x', { duration: 1.2, ease: 'power2.out' }),
    }));
    $('.hero').addEventListener('pointermove', (e) => {
      const nx = e.clientX / window.innerWidth - 0.5;
      layers.forEach((l) => l.to(nx * -l.k));
    });
  }

  /* ---------------- Form ---------------- */
  const fields = $$('.field input, .field textarea, .field select');
  const syncFields = () => fields.forEach((f) => f.classList.toggle('has-val', !!f.value));
  fields.forEach((f) => { f.addEventListener('input', syncFields); f.addEventListener('change', syncFields); });
  syncFields();
  /* back-navigation restores values without firing input events */
  ...
  form?.addEventListener('submit', (e) => {
    ...
    gsap.to(form, {
      ...
      /* display:none, not [hidden] — .form's display:flex outranks the UA rule.
         Collapsing it also lifts the success message into view on mobile. */
      onComplete: () => { form.style.display = 'none'; success.focus(); },
    });
  });
  ...
  if (mcta) {
    ...
    ScrollTrigger.create({
      trigger: '#order', start: 'top 85%', end: 'bottom top',
